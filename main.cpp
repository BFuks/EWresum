...
double Real_Gluon(double*,size_t,void*);                                     //
double Real_Quark(double*,size_t,void*);                                     //
...
int main(int argc, char* argv[])
{
  ...
  info("  --> LO component");
  //Integrate(&Born,res,err,ndims,Params);
  DisplayXsec(res, err, "  --> LO component",0);
  r_nlo=res; e_nlo=err;
  ...
  DisplayXsec(res, err, "  --> \"Virtual + dipole\" component",0);
  r_nlo+=res; e_nlo+=err;

  info("  --> Colinear reminder (P+K)");
  //Integrate(&PK,res,err,ndims+1,Params);
  DisplayXsec(res, err, "  --> \"P+K\" component",0);
  r_nlo+=res; e_nlo+=err;

  info("  --> \"Real - dipole\" component [qqbar]");
  // Integrate(&Real_Gluon,res,err,ndims+3,Params);
  DisplayXsec(res, err, "  --> \"Real - dipole\" component [qqb]",0);
  r_nlo+=res; e_nlo+=err;

  info("  --> \"Real - dipole\" component [qg]");
  Integrate(&Real_Gluon,res,err,ndims+3,Params);
  DisplayXsec(res, err, "  --> \"Real - dipole\" component [qg]",0);
  r_nlo+=res; e_nlo+=err;
